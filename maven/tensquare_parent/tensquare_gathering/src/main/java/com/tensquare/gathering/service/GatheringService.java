...
public class GatheringService {
     ...
     * @return
     */
    //@Cacheable:添加缓存,按照方法为单位进行添加的
    //value:使用的哪个缓存空间,自己的缓存,value要一样,名字任意
    //key和方法一起进行缓存查询,不同的参数使用的是不同的缓存数据
        ...
        return new Specification<Gathering>() {
            ...
            public Predicate toPredicate(Root<Gathering> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
                ...
                if (searchMap.get("detail") != null && !"".equals(searchMap.get("detail"))) {
                    predicateList.add(cb.like(root.get("detail").as(String.class), "%" + (String) searchMap.get
                            ("detail") + "%"));
                }
                // 主办方
